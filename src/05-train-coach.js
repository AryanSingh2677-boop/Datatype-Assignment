...
export function findPassenger(passengers, name) {
  ...
  let result = passengers.find(passenger => {
    return (
      typeof passenger.name === "string" && passenger.name.toLowerCase() === name.toLowerCase()
    )
  })
...
export function getPassengerIndex(passengers, name) {
  ...
  let passengerseatno = passengers.findIndex(passenger => {
    return (
...
export function isAnyWaitlisted(passengers) {
  ...
  let waitlist = passengers.some(passenger => {
    return (
...
export function areAllConfirmed(passengers) {
  ...
  let confirmed = passengers.every(passenger => {
    return (
...
export function getWaitlistedPassengers(passengers) {
  ...
  let waitingListPassenger = passengers.filter(passenger => {
    return (
      passenger.status === "waitlisted"
    )
  })
  return waitingListPassenger
}


console.log(findPassenger([{name:"Rahul",coach:"S5",seat:42,status:"confirmed"}], "rahul"));

console.log(getPassengerIndex([{name:"Rahul"}, {name:"Priya"}], "priya"));

console.log(isAnyWaitlisted([]));

console.log(areAllConfirmed([]));

console.log(getWaitlistedPassengers([{name:"A",status:"confirmed"},{name:"B",status:"waitlisted"} , {name:"C" , status:"waitlisted"}]));
