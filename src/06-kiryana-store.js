...
export function getAffordableItems(items, maxPrice) {
  ...
  let maxPricing = items.filter((item , totalprice) => {
    return ( 
      item.price <= maxPrice 
    )
  })
...
export function calculateTotal(items) {
  ...
  let costPrice = items.reduce((totalprice , item )=> {
    return (
...
export function sortByPrice(items, ascending) {
  ...
  let newSortedArr = [...items].sort((a,b)=> {
    if(ascending == true ){
...
export function formatBill(items) {
  ...
  let  printedbill = items.map((item)=> {
    return (
    printing = `${item.name} x ${item.qty} = Rs.${item.price * item.qty}`
  )}).join("\n")

  return printedbill
}

console.log(getItemNames([{name:"Atta",price:40,qty:2}]));

console.log(getAffordableItems([{name:"Atta",price:40},{name:"Ghee",price:500}],100));

console.log(calculateTotal([{name:"Atta",price:40,qty:2},{name:"Daal",price:80,qty:1}]));

console.log(sortByPrice([{name:"Ghee",price:500},{name:"Atta",price:40}], true));
