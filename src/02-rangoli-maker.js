...
export function repeatPattern(pattern, times) {
  ...
  let newpattern = pattern.repeat(times);
...
export function extractRangoliCenter(design, start, end) {
  ...
  let cutOutDesign = design.slice(start, end)
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  let color = colorString.split(oldSep)

  let newcolor = color.join(newSep)
...
export function replaceRangoliColor(design, oldColor, newColor) {
  ...
  let givenRangoliColor = design.replaceAll(oldColor,newColor)



  return givenRangoliColor
}
...
console.log(extractRangoliCenter("@@@@LoTUs@@@@" , 4 , 9));

console.log(splitAndJoinRangoli("red,blue,green", ",", " / "));

console.log(replaceRangoliColor("red-blue-red-green-red", "blue", "yellow"));
