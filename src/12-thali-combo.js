...
export function createThaliDescription(thali) {
  ...
  const cuisineChoice = thali.isVeg ? "Veg" : "Non-Veg";

  return `${thali.name.toUpperCase()} (${cuisineChoice}) - Items: ${thali.items.join(", ")} - Rs.${thali.price.toFixed(2)}`;
}
...
export function generateThaliReceipt(customerName, thalis) {
  ...
  if (
    typeof customerName !== "string" ||!Array.isArray(thalis) ||thalis.length === 0
  ){
    return "";
  }
  ...
  const receipt = `THALI RECEIPT Customer: ${upperName} ${lineItems} Total: Rs.${total} Items: ${count}`;
...
console.log(createThaliDescription({name:"Rajasthani Thali", items:["dal","churma"], price:250, isVeg:false}));
