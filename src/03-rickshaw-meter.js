...
export function parseFare(fareString) {
  // Your code here
  if(typeof fareString != "string" || fareString === "" || isNaN(fareString)){
    return -1 
  }
  let parseprice = parseFloat(fareString)
...
export function roundFare(amount, decimalPlaces) {
  ...
  let roundedprice = amount.toFixed(decimalPlaces);
  return roundedprice ;
}
...
export function calculateSurge(baseFare, surgeMultiplier) {
  ...
  let price = baseFare * surgeMultiplier

  let finalFare = Math.ceil(price)
...
export function getDistanceDifference(from,to) {
  // Your code here
  let StartDistance = parseInt(from)
  let traveledDistance = parseInt(to)

  if(isNaN(StartDistance) || isNaN(traveledDistance)){
    return -1
  }
  let abosulteDistance = Math.abs(StartDistance - traveledDistance)
  return abosulteDistance
}


console.log(parseFare("0978234.23"));

console.log(roundFare(43222.234,23.3));

console.log(calculateSurge("34",2.3));

console.log(findCheapestAndCostliest(34,2423, 9));

console.log(getDistanceDifference(24 , "sd"))